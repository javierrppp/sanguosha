...
function SmartAI:findLijianTarget(card_name, use)
	...
	if not self.player:hasUsed(card_name) then
		...
		local males, others = {}, {}
		local first, second
		local zhugeliang_kongcheng, xunyu, xushu

		for _, enemy in ipairs(self.enemies) do
			if enemy:isMale() then
				if enemy:hasShownSkill("wuyan") then xushu = enemy
				elseif enemy:hasShownSkill("kongcheng") and enemy:isKongcheng() then zhugeliang_kongcheng = enemy
					...
					for _, anotherenemy in ipairs(self.enemies) do
						if anotherenemy:isMale() and anotherenemy:objectName() ~= enemy:objectName() then
							...
							if #males == 1 and self:hasTrickEffective(duel, males[1], anotherenemy) then
								if not anotherenemy:hasShownSkills("jizhi|jiang|wuyan") then
									if males[1]:hasShownSkill("fuli") then
										local big_kingdoms = males[1]:getBigKingdoms("fuli")
										if not table.contains(big_kingdoms,anotherenemy:getKingdom()) then
