...
function SmartAI:askForPindian(requestor, reason)
	...
	--[[if self.player:hasSkill("zongshi") then
		local maxNum = 0
		local spadeCard = nil
		for _, c in ipairs(cards) do
			if c:getSuit() == sgs.Card_Spade and c:getNumber() > maxNum then
				maxNum = c:getNumber()
				spadeCard = c
			elseif c:getSuit() == sgs.Card_Spade and c:getNumber() == maxNum then
				if spadeCard and self:getUseValue(c) < self:getUseValue(spadeCard) then
					spadeCard = c 
				end
			end
		end
		if spadeCard then
			if spadeCard:getNumber() < self:getMaxCard():getNumber() -2 then
				if self:getMaxCard():getNumber() < 7 then
					return spadeCard
				else
...
function SmartAI:damageIsEffective_(damageStruct)
	...
	if to:hasShownSkill("fuli") and from and table.contains(big_kingdoms,from:getKingdom()) then 
		...
		if (from:getEquip(0) and from:getEquip(0):isKindOf("Fan")) or self:getCardsNum("FireSlash") > 0 or self:getCardsNum("FireAttack") > 0 then
			if to:getEquip(1) and to:getEquip(1):isKindOf("Vine") then return true end
		end
...
function SmartAI:hasTrickEffective(card, to, from)
	...
	if to:hasShownSkill("duliang") and (card:isKindOf("Dismantlement") or card:isKindOf("SupplyShortage")) then
...
dofile "lua/ai/shopEquip-ai.lua"
