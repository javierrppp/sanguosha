...
sgs.ai_skill_cardask["slash-jink"] = function(self, data, pattern, target)
	...
	if self.player:hasSkill("fuli") then
		local big_kingdoms = self.player:getBigKingdoms("fuli")
		if table.contains(big_kingdoms, target:getKingdom()) then return "." end
	end
	if self.player:getMark("@fu") > 0 then
		local xizhicai = sgs.findPlayerByShownSkillName("xianfu")
		if self:isEnemy(xizhicai) and self:isWeak(xizhicai) and not self:isWeak(self.player) then
			return "."
		end
	end
...
function SmartAI:useCardPeach(card, use)
	...
	if self.player:hasSkill("kuangcai") and self.player:getMark("@kuang") > 0 then
		use.card = card
		return
	end
...
sgs.ai_skill_cardask.aoe = function(self, data, pattern, target, name)
	...
	if self.player:hasSkill("fuli") then
		local big_kingdoms = self.player:getBigKingdoms("fuli")
		if table.contains(big_kingdoms, attacker:getKingdom()) then return "." end
	end
	if self.player:getMark("@fu") > 0 then
		local xizhicai = sgs.findPlayerByShownSkillName("xianfu")
		if self:isEnemy(xizhicai) and self:isWeak(xizhicai) and not self:isWeak(self.player) then
	...
	if attacker:hasShownSkill("wuyan") then return "." end
...
sgs.ai_skill_cardask["duel-slash"] = function(self, data, pattern, target)
	...
	if self.player:hasSkill("wuyan") then return "." end
	if target:hasShownSkill("wuyan") then return "." end
...
sgs.ai_use_priority.ExNihilo = 9
