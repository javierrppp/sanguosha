...
sgs.ai_skill_invoke.keji = function(self, data)
	...
	if self.player:hasSkill("yinbing") then
		if (room:findPlayerByShownSkillName("guzheng") or self.player:getHandcardNum() <= self.player:getMaxCards()) and self.player:isWounded() then
			return false
		end
	end
	return true
